...
class TestPositive:
    ...
    def test_step4(self, start_time):
        ...
        assert ssh_checkout(data["ip"], data["user"], data["passwd"], 
                            "cd {}; 7z t" " arx2.7z".format(data["folder_out"]),
    ...
    def test_step5(self, start_time):
        ...
        assert ssh_checkout(data["ip"], data["user"], data["passwd"], 
                            "cd {}; 7z u" " arx2.7z".format(data["folder_in"]),
    ...
    def test_step6(self, clear_folders, make_files, start_time):
        ...
        for item in make_files:
            res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], 
                                    "cd {}; 7z l" " arx2.7z".format(data["folder_out"], data["folder_ext"]), item))
    ...
    def test_step7(self, clear_folders, make_files, make_subfolder, start_time):
        ...
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], 
                                ...
                                "Everything is Ok"))
        res.append(ssh_checkout(data["ip"], data["user"], data["passwd"], 
                                "cd {};" " 7z x arx.7z -o{} -y".format(data["folder_out"], data["folder_ext2"]), 
                                "Everything is Ok"))
        for item in make_files:
            res.append(
...
# Запуск из консоли: pytest -v test_01.py
